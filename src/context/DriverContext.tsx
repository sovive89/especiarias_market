...
import type { Delivery, DeliveryEvent, DeliveryProblemType, Driver, Route } from "../types/delivery";
...
interface DriverState {
  ...
  stats: { total: number; done: number; open: number; problems: number; remainingKm: number; remainingMin: number };
...
export function DriverProvider({ children }: { children: ReactNode }) {
  ...
  const reportProblem = useCallback(
    async (id: string, type: DeliveryProblemType, description: string) => {
      const ok = await run(() => deliveryService.reportProblem({ deliveryId: id, type, description }));
  ...
  }, [loading, busy, error, driver, route, deliveries, events, selectedId, sheet, toggleAvailability, startRoute, finishRoute, advance, reportProblem]);
