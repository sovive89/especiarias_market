...
export type DeliveryStatus = "ASSIGNED" | "READY" | "IN_ROUTE" | "ARRIVED" | "DELIVERED_BY_DRIVER" | "CONFIRMED_BY_CUSTOMER" | "PROBLEM" | "CANCELLED";
...
export interface Driver { id: string; name: string; phone: string; status: DriverStatus; }
export interface Delivery {
  id: string; orderId: string; driverId: string; routeId: string; sequence: number; status: DeliveryStatus;
  customerName: string; customerPhone: string; address: string; complement?: string; notes?: string;
  latitude?: number; longitude?: number; trackingEnabled: boolean; trackingToken?: string;
