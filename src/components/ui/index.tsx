...
export function Button({ variant = "primary", size = "md", block, className, type = "button", ...props }: ButtonProps) {
  return (
    <button
      type={type}
      className={cn("btn", `btn--${variant}`, size === "lg" && "btn--lg", block && "btn--block", className)}
      {...props}
    />
  );
}
...
export function IconButton({ label, className, children, ...props }: ButtonHTMLAttributes<HTMLButtonElement> & { label: string }) {
  return (
    <button type="button" aria-label={label} title={label} className={cn("icon-btn", className)} {...props}>
      {children}
    </button>
  );
}
...
export function Stat({ icon, value, label, tone = "neutral" }: { icon: ReactNode; value: ReactNode; label: string; tone?: Tone }) {
  return (
