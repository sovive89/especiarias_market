...
export function ProblemSheet({ delivery }: { delivery: Delivery }) {
  ...
  return (
    <Sheet eyebrow="Ocorrência" title="Registrar problema" onClose={() => openDelivery(delivery.id)}>
      ...
      <label className="field">
        ...
        <textarea value={description} onChange={e => setDescription(e.target.value)} placeholder="Descreva o que aconteceu" rows={3} />
      </label>

      <div className="sheet__actions">
        <Button size="lg" block disabled={!canSave || busy} onClick={() => type && reportProblem(delivery.id, type, description.trim())}>
