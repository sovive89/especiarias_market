      ...
      <button type="button" className="delivery-card__main" onClick={onOpen}>
        ...
        <span className="delivery-card__body">
          ...
          <span className="delivery-card__meta mono">
            <span><Clock3 size={13} aria-hidden />{delivery.etaMinutes ?? "--"} min</span>
            <span>{delivery.distanceKm !== undefined ? formatKm(delivery.distanceKm) : "-- km"}</span>
          </span>
        </span>
        <ChevronRight size={18} className="muted" aria-hidden />
      </button>
      <a className="icon-btn icon-btn--soft" href={mapsUrl(delivery.address)} target="_blank" rel="noreferrer" aria-label={`Navegar até ${delivery.customerName}`}>
        <Navigation size={18} />
      </a>
    </div>
  );
