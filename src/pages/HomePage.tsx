import { AlertTriangle, CheckCircle2, Clock3, Flag, MapPin, Navigation, PackageCheck, PartyPopper, Power } from "lucide-react";
...
import { Badge, Button, EmptyState, PageHeader, SectionTitle, Stat, Surface } from "../components/ui";
...
export function HomePage() {
  const { driver, stats, nextDelivery, deliveries, available, routeStarted, routeFinished, busy, toggleAvailability, startRoute, finishRoute, openDelivery } = useDriver();
  ...
  return (
    <>
      <PageHeader
        ...
        subtitle={routeFinished ? "Rota de hoje finalizada." : routeStarted ? "Rota em andamento." : "Sua rota de hoje está pronta."}
        action={
          <button
            type="button"
      ...
      <Surface className="route-progress">
        <div className="route-progress__top">
          <div>
            ...
            <p className="route-progress__count">
              {stats.done}<span className="muted">/{stats.total} entregas</span>
            </p>
          </div>
        ...
        {!available && !routeStarted && !routeFinished && <p className="hint">Fique disponível para iniciar a rota.</p>}
      </Surface>
      ...
      <div className="stats-grid">
        ...
        <Stat icon={<AlertTriangle size={18} />} value={stats.problems} label="Problemas" tone={stats.problems ? "danger" : "neutral"} />
      </div>

      <SectionTitle action={<Link to="/entregas" className="link">Ver todas</Link>}>Próxima entrega</SectionTitle>

      {nextDelivery ? (
        <Surface className="next-delivery">
          <button type="button" className="next-delivery__head" onClick={() => openDelivery(nextDelivery.id)}>
            <span>
              <span className="mono muted">Parada {pad2(nextDelivery.sequence)} · {nextDelivery.orderId}</span>
          ...
          <div className="next-delivery__steps">
            <div>
              ...
              <span>
                <strong>Coleta · Especiarias</strong>
            ...
            <div>
              ...
              <span>
                ...
                <small className="muted">
                  {nextDelivery.distanceKm !== undefined && formatKm(nextDelivery.distanceKm)} · cerca de {nextDelivery.etaMinutes ?? "--"} min
                </small>
              </span>
            </div>
          </div>
        ...
        <Surface>
          <EmptyState icon={<PartyPopper size={24} />} title={routeFinished ? "Rota finalizada!" : "Nenhuma entrega em aberto"} text={routeFinished ? "Bom trabalho. Até a próxima rota." : "Finalize a rota para encerrar o dia."} />
        </Surface>
      )}
      ...
      {upcoming.length > 0 && (
        <>
          ...
          <div className="list">
            {upcoming.map(d => <DeliveryCard key={d.id} delivery={d} onOpen={() => openDelivery(d.id)} />)}
          </div>
        </>
      )}
    </>
