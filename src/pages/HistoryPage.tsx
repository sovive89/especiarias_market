import { AlertTriangle, CheckCircle2, Clock3, Flag, History, MapPin, Navigation, Truck, type LucideIcon } from "lucide-react";
...
export function HistoryPage() {
  ...
  return (
    <>
      <PageHeader eyebrow="Histórico" title="Atividade de hoje" subtitle="Tudo o que aconteceu na sua rota." />

      <div className="stats-grid stats-grid--2">
        <Stat icon={<CheckCircle2 size={18} />} value={stats.done} label="Entregas concluídas" tone="good" />
      ...
      {events.length ? (
        <Surface as="section" className="timeline">
          <ol>
            {events.map(ev => {
              ...
              return (
                <li key={ev.id} className={cn("timeline__item", ev.type === "DELIVERY_PROBLEM" && "is-problem", ev.type === "DELIVERY_COMPLETED" && "is-done")}>
                  ...
                  <time className="mono muted" dateTime={ev.createdAt}>{formatTime(ev.createdAt)}</time>
                </li>
              );
            })}
          </ol>
        ...
        <Surface>
          <EmptyState icon={<History size={24} />} title="Sem atividade ainda" text="Inicie a rota e cada etapa aparece aqui." />
        </Surface>
      )}
    </>
  );
