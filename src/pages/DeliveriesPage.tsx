...
export function DeliveriesPage() {
  ...
  return (
    <>
      <PageHeader eyebrow="Entregas" title="Suas entregas" subtitle={`${deliveries.length} entregas na rota de hoje`} />
      <Chips label="Filtrar entregas" options={options} value={filter} onChange={setFilter} />

      {visible.length ? (
        <div className="list">
          {visible.map(d => <DeliveryCard key={d.id} delivery={d} onOpen={() => openDelivery(d.id)} />)}
        </div>
      ) : (
        <EmptyState icon={<PackageSearch size={24} />} title="Nada por aqui" text="Nenhuma entrega com este filtro." />
      )}
    </>
  );
}
